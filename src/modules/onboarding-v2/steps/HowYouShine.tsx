...
export default function HowYouShine({ router }: Props) {
  ...
  const selected = signals.shineKeys ?? [];
  const max = 3;

  const toggle = (trait: string) => {
    const has = selected.includes(trait);
    if (has) {
      updateSignals({ shineKeys: selected.filter((x) => x !== trait) });
    } else if (selected.length < max) {
      updateSignals({ shineKeys: [...selected, trait] });
    }
    // Silent ignore if at max - no error or feedback
  };

  const handleKeyDown = (e: React.KeyboardEvent, trait: string) => {
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-white">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-3">How you shine</h1>
        <p className="text-gray-600">Pick up to {max} traits that describe you best.</p>
      </div>

      <div className="grid grid-cols-1 gap-3">
        {pack.shine.map((trait) => {
